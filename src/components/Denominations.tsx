...
function DeleteModal({ denomination, onClose, onConfirm }: DeleteModalProps) {
  return (
...
export function Denominations() {
  ...
  const handleDeleteConfirm = () => {
    if (selectedDenomination) {
      setDenominations(denominations.filter(d => d.id !== selectedDenomination.id));
  ...
  const totalPages = Math.ceil(filteredDenominations.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedDenominations = filteredDenominations.slice(startIndex, startIndex + itemsPerPage);
            ...
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {Math.min(startIndex + itemsPerPage, filteredDenominations.length)}
                </span>{' '}
                of <span className="font-medium">{filteredDenominations.length}</span> results
              </p>
            </div>
            <div>
